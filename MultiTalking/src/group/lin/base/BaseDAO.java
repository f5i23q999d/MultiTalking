/**
 * 项目名：student
 * 修改历史：
 * 作者： MZ
 * 创建时间： 2016年1月6日-上午10:04:37
 */
package group.lin.base;

import java.sql.ResultSet;
import java.sql.SQLException;

import group.multiTalking.DAO;
import group.lin.dao.SendDAO;
import group.lin.dao.ReceiveDAO;
import group.lin.dao.LoginDAO;
import group.lin.util.DBUtil;

/**
 * 模块说明： DAO基类
 * 
 */
public abstract class BaseDAO {
	protected final DBUtil db = DBUtil.getDBUtil();
	protected ResultSet rs;
	private static BaseDAO baseDAO;

	public BaseDAO() {
		init();
	}

	private void init() {
		// buildAbilityDAO();
	}

	// protected abstract void buildAbilityDAO();

	public static synchronized BaseDAO getAbilityDAO(DAO dao) {
		switch (dao) {
		case SendDAO:
			if (baseDAO == null || baseDAO.getClass() != SendDAO.class) {
				baseDAO = SendDAO.getInstance();
			}
			break;
		case ReceiveDAO:
			if (baseDAO == null || baseDAO.getClass() != ReceiveDAO.class) {
				baseDAO = ReceiveDAO.getInstance();
			}
			break;
		case LoginDAO:
			if (baseDAO == null || baseDAO.getClass() != LoginDAO.class) {
				baseDAO = LoginDAO.getInstance();
			}
			break;
		default:
			break;
		}
		return baseDAO;
	}

	protected void destroy() {
		try {
			if (rs != null) {
				rs.close();
			}
		} catch (SQLException se) {
			se.printStackTrace();
		} finally {
			db.close();
		}
	}
}
